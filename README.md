...
Distinct MRC-branded static site for board-level PlayStation, Xbox and Nintendo Switch / handheld console repair and used-console verification at [consolerepair.ca](https://consolerepair.ca/).

The production artifact is generated into `_site/`. It contains only the console-focused site; the legacy `docs/` tree is retained as an unpublished architectural source snapshot and is never uploaded by the Pages workflow. The sibling `motherboardrepair-ca` and `graphicsrepair-ca` repositories are read-only architectural references.
...
- a `consolerepair.ca` CNAME, sitemap and robots policy;
...
GitHub Actions builds `_site/` on `ubuntu-latest` and deploys it with the official GitHub Pages artifact workflow from `main`. Run:
