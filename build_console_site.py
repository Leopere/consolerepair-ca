#!/usr/bin/env python3
"""Build the production-only Console Repair Canada static artifact."""
...
SOURCE = ROOT / "site"
OUTPUT = ROOT / "_site"
DOMAIN = "https://consolerepair.ca"
NOTOMO_ORIGIN = "https://notomo.colinknapp.com"
NOTOMO_SITE_ID = "consolerepair.ca"
    ...
    <header class="site-header">
      <div class="shell header-row">
        <a class="brand" href="/{locale_path(locale)}">
          ...
          <span><strong>Console Repair</strong><small>Canada</small></span>
        </a>
        ...
        "@graph": [
            {
                ...
                "name": "Console Repair Canada", "alternateName": "MRC",
...
<head>
  ...
  {alternates()}
  <meta property="og:type" content="website"><meta property="og:site_name" content="Console Repair Canada"><meta property="og:title" content="{esc(c['title'])}"><meta property="og:description" content="{esc(c['description'])}"><meta property="og:url" content="{canonical}"><meta property="og:image" content="{DOMAIN}/assets/console-repair.webp">
...
<main id="main">
  <section class="hero">
    <div class="hero-art" aria-hidden="true"><img src="{prefix}assets/console-repair-720.webp" srcset="{prefix}assets/console-repair-480.webp 480w, {prefix}assets/console-repair-720.webp 720w" sizes="(max-width: 920px) 100vw, 51vw" width="720" height="720" alt="" fetchpriority="high" decoding="async"></div>
    ...
    </div><div class="board-card" aria-hidden="true"><span>CONSOLE</span><strong>TEST / REPAIR</strong><small>PlayStation · Xbox · Switch</small></div></div>
  </section>
...
def render_legal(kind: str) -> str:
    if kind == "privacy":
        ...
        description = "How Console Repair Canada handles contact information, repair details, hosting metrics and service records."
        body = """
        <h2>Information we collect</h2><p>When you submit the repair form, we receive the name, email, phone number and the phone format used to validate it, console manufacturer and model, drop-off or mail-in choice, rush service choice, symptoms and repair history you provide. For a mail-in request, we also receive the return country, province, state or region, return address and optional unit number you provide. For an international mail-in request, we receive your ownership or owner-authorization confirmation and your acknowledgement of cross-border shipping costs and instructions. We use this information to review your request, contact you and carry out the work if we agree to take the job. We do not ask for the console serial number.</p>
        ...
        title = "Service Terms"
        description = "Basic request terms for Console Repair Canada."
        body = """
        <h2>Sending a request</h2><p>Console Repair Canada is an MRC specialist site. Submitting the form sends a request for review; it does not reserve a time, authorize work or guarantee a repair. Canada is our main market. International mail-in service is available only for jobs MRC accepts. Do not mail a console until MRC provides shipping instructions.</p>
        ...
        <h2>Cleaning and thermals</h2><p>Cleaning and thermals start at $90 plus tax for one supported PlayStation, Xbox, Nintendo Switch or handheld console that MRC has agreed to take. We open the console, clean the dust and residue we can reach, and replace accessible heat-transfer materials. Extra work, parts, shipping and tax are quoted separately. International clients are billed in USD.</p><p>This service is not a repair diagnostic. It does not guarantee lower temperatures, quieter fans, a fixed fault or that there are no hidden faults. Do not mail a console until MRC agrees to take it and provides shipping instructions.</p>
        <h2>Rush service</h2><p>Rush service is optional and costs an additional $130.</p>
        <h2>Reviewing your request and quoting repairs</h2><p>We review your request for free to decide whether we can take the job. This review is not a repair diagnostic. Once a console we have agreed to work on arrives, we diagnose it and give you a quote. No repair work begins until you approve that quote. The work, price and other details MRC gives you for your job are the ones that apply.</p>
        """
...
def build() -> None:
    ...
    for source in (SOURCE / "assets").iterdir():
        if source.is_file() and not source.name.startswith("gpu-"):
    ...
    asset_map = {
        ...
        ROOT / "docs/images/console_repair.webp": OUTPUT / "assets/console-repair.webp",
    ...
    (OUTPUT / "CNAME").write_text("consolerepair.ca\n", encoding="utf-8")
...
def main() -> None:
    ...
    build()
    if args.check:
        from tests.test_console_site import validate_site
        validate_site()
