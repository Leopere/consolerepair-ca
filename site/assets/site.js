(function () {
  ...
  function setupLanguages() {
    document.querySelectorAll('[data-language]').forEach(function (link) {
      link.addEventListener('click', function () {
        try { localStorage.setItem('consolerepair-language', link.dataset.language); } catch (error) {}
      });
    });
    if (document.body.dataset.locale !== 'en' || location.pathname !== '/') return;
    let selected = '';
    try { selected = localStorage.getItem('consolerepair-language') || ''; } catch (error) {}
  ...
  function setupRepairPrompt() {
    ...
    const dismissKey = 'consolerepair-repair-prompt-dismissed';
