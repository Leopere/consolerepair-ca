#!/usr/bin/env python3
"""Production artifact checks for Console Repair Canada."""
...
def validate_site() -> None:
    assert (SITE / "CNAME").read_text(encoding="utf-8") == "consolerepair.ca\n"
    ...
    for locale, tag in LOCALES.items():
        ...
        assert source.count('data-site-id="consolerepair.ca"') == 1
        ...
        assert "connect-src 'self' https://forms.motherboardrepair.ca https://notomo.colinknapp.com/collect https://notomo.colinknapp.com/replay https://notomo.colinknapp.com/n-config/consolerepair.ca" in source
        ...
        expected_url = "https://consolerepair.ca/" if locale == "en" else f"https://consolerepair.ca/{locale}/"
    ...
    assert (SITE / "assets" / "console-repair-480.webp").stat().st_size < 60_000
    assert (SITE / "assets" / "console-repair-720.webp").stat().st_size < 100_000
    ...
    assert "console_model: model" in js
    assert "graphics_card_model" not in js
    assert "source_site: 'consolerepair.ca'" in js
    ...
    assert 'data-site-id="consolerepair.ca"' in not_found
    ...
    for legal_kind in ("privacy", "terms"):
        ...
        assert 'data-site-id="consolerepair.ca"' in legal_source
    ...
    assert "self-hosted" not in deploy + build_workflow
    assert "playwright" not in deploy + build_workflow
    assert "test_wasm_browser" not in deploy + build_workflow
    assert (deploy + build_workflow).count("runs-on: ubuntu-latest") == 3
    assert "python3 build_console_site.py --check" in deploy
    assert "python3 build_console_site.py --check" in build_workflow

    sitemap = ET.parse(SITE / "sitemap.xml").getroot()
    namespace = {"s": "http://www.sitemaps.org/schemas/sitemap/0.9"}
    urls = {node.text for node in sitemap.findall("s:url/s:loc", namespace)}
    assert len(urls) == 8
    assert "https://consolerepair.ca/" in urls
    assert "https://consolerepair.ca/privacy/" in urls
    assert "https://consolerepair.ca/terms/" in urls
    assert all(url and url.startswith("https://consolerepair.ca/") for url in urls)


def test_production_artifact() -> None:
    subprocess.run(["python3", str(ROOT / "build_console_site.py")], cwd=ROOT, check=True)
    validate_site()


if __name__ == "__main__":
    test_production_artifact()
    print("✓ distinct console production artifact")
