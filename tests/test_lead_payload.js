...
test('French Quebec lead preserves the original message and sends French/location metadata', async () => {
  const originalMessage = 'Bonjour!\n\nMa PlayStation 5 <ne démarre plus>.  Pouvez-vous\nme donner un devis?';
  const form = fakeForm({
  ...
  assert.equal(leadApiBody.form_id, 'console_repair_quote');
  assert.equal(leadApiBody.extra_fields.console_model, 'PlayStation 5');
  assert.equal(leadApiBody.extra_fields.source_site, 'consolerepair.ca');
...
test('English Quebec lead preserves the original message without English duplication or preference metadata', async () => {
  const originalMessage = 'Hello!\n\nMy Xbox Series X has <no HDMI output>.  Please send a quote.';
  const form = fakeForm({
    message: originalMessage,
    province: 'QC',
  });
...
for (const serviceType of ['Mail-In', 'In-Person']) {
  for (const rush of [false, true]) {
    test(`${serviceType} request ${rush ? 'includes optional rush for $130' : 'has no rush fee by default'}`, async () => {
      const message = 'Please help with cleaning and thermals for my PlayStation 5.';
      ...
      assert.equal(Object.hasOwn(body.extra_fields, 'request_type'), false);
...
test('unchecking rush removes its fee from the next submission', () => {
  const form = fakeForm({ message: 'My console needs a repair.', province: 'ON', rush: true });
